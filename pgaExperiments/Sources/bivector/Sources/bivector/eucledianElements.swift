...
public func idealPoint<A:Numeric>(_ p:Point<A>) -> [(A, [e])] {
  p
  |> set(^\Point.c , (0, []))
  |> point
}

public func direction<A:Numeric>(_ p:Point<A>) -> [(A, [e])] {
  p |> idealPoint
}

public func point<A:Numeric>(_ p:Point<A>) -> [(A, [e])] {
  p.x |+| p.y |+| p.z |+| p.c
}
...
public func plane<A:Numeric>(_ p:Plane<A>) -> [(A, [e])] {
  p.a |+| p.b |+| p.c |+| p.d
}
...
public func getPoint<A:Numeric>(x:A,y:A,z:A,c:A=1) -> Point<Double> {
