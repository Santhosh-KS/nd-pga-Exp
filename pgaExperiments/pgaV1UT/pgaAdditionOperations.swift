...
//  var e0:e { epsilons.first! }
  ...
  func testBasiVectorCreationUsing2ArraysOfEpsilons() {
    ...
    let validateResults1:[(Double,e)] = reduce(with: |+|,zip2(with: |+|)(epsilons1, epsilons2) |> flatmap)
  ...
  func testBasisVectorCreationUsingBasisVectorAndFloat() {
    ...
    let result = (2.1, e(1)) |+| 10
    
    XCTAssertEqual(result.count, 2)
    XCTAssertEqual(result.last!.0, 10)
    XCTAssertEqual(result.last!.1, e(0))
    
    XCTAssertEqual(result.first!.0, 2.1)
    XCTAssertEqual(result.first!.1, e(1))
  }
  ...
  func testBasisVectorCreationUsingArrayOfFloatsAndArrayOfBasisVectors() {
    ...
    let validateResults = reduce(with: |+|,zip2(with: |+|)(f1, bv1) |> flatmap)
    ...
    let validateResults1 = reduce(with: |+|, zip2(with: |+|)(arrayOfFloats, arrayOfBasisVectors) |> flatmap)
    let _ = zip2(validateResults1, result1).map { pairs in
      let validating:(coef:Float, E:e) = pairs.0
      let result:(coef:Float, E:e) = pairs.1
      XCTAssertEqual(validating.coef, result.coef)
  ...
  func testBasisVectorCreationUsingArrayOfBasisVectorsAndArrayOfFloats() {
    ...
    let validateResults = reduce(with: |+|,
                                 zip2(with: |+|)(bv1, f1) |> flatmap)
    ...
    let validateResults1 = reduce(with: |+|,
                                  zip2(with: |+|)(arrayOfBasisVectors, arrayOfFloats) |> flatmap)
...
//    XCTAssertEqual(result.last!, result1.last!.1)
...
//    let r1 = (Float(1.1),e(1)) |+| (Float(2.2), e(2)) |+| bv1
    ...
    let validateResults1 = reduce(with: |+|,
                                  zip2(with: |+|)(arrayOfBasisVectors, arrayOfBasisVectors1) |> flatmap)
    let _ = zip2(validateResults1, result1).map { pairs in
      let validating:(coef:Float, E:e) = pairs.0
      let result:(coef:Float, E:e) = pairs.1
      XCTAssertEqual(validating.coef, result.coef)
  ...
  func testBasisVectorCreationUsingArrayOfBasisVectorAndArrayOfFloats() {
    ...
    let validateResults1 = reduce(with: |+|,
                                  zip2(with: |+|)(arrayOfBasisVectors1, arrayOfFloats) |> flatmap)
    let _ = zip2(validateResults1, result1).map { pairs in
      let validating:(coef:Float, E:e) = pairs.0
      let result:(coef:Float, E:e) = pairs.1
      XCTAssertEqual(validating.coef, result.coef)
...
//    let basisVector = e(1)
//    let arrayOfBasisVector = [1.1 |^| e(1), 2.2 |^| e(2), 3.3 |^| e(3)]
//    // (1, e(1)) |+| [1.1 |^| e1, 2.2 |^| e2, 3.3 |^| e3] = [(A, [e])]
